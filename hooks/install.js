...
function installHooks() {
  console.log(chalk.blue('\n🔧 Installing QA test hooks...\n'));
  
  // Check if we're in a git repository
  const gitDir = path.join(process.cwd(), '.git');
  if (!fs.existsSync(gitDir)) {
    console.log(chalk.yellow('⚠️  No .git directory found. Skipping hook installation.'));
...
# Auto-generated by @bateriku/qa-scripts
# This hook runs QA tests before allowing a commit

set -e

echo "🧪 Running QA tests before commit..."

# Check if qa-scripts is installed
if ! npm list @bateriku/qa-scripts >/dev/null 2>&1; then
  echo "⚠️  @bateriku/qa-scripts not found. Installing..."
  npm install --save-dev @bateriku/qa-scripts
fi

# Update to latest version (optional - comment out if you want to control versions)
# npm update @bateriku/qa-scripts
...
if [ $? -ne 0 ]; then
  echo ""
  echo "❌ QA tests failed! Please fix the issues before committing."
...
echo "🧪 Running comprehensive QA tests before push..."
...
if [ $? -ne 0 ]; then
  echo ""
  echo "❌ QA tests failed! Please fix the issues before pushing."
...
`;
  ...
  if (existingPreCommit) {
    ...
    console.log(chalk.yellow(`⚠️  Existing pre-commit hook backed up to: ${path.basename(backupPath)}`));
  }
  ...
  console.log(chalk.green.bold('\n✅ QA test hooks installed successfully!\n'));
  console.log(chalk.gray('   The following npm scripts are now available:'));
  console.log(chalk.cyan('   • npm run qa:test         - Run tests for this project'));
  console.log(chalk.cyan('   • npm run qa:test:local   - Run tests with local environment'));
  console.log(chalk.cyan('   • npm run qa:test:staging - Run tests with staging environment'));
  console.log(chalk.cyan('   • npm run qa:test:headed  - Run tests in headed mode (visible browser)'));
  console.log(chalk.cyan('   • npm run qa:update       - Update qa-scripts to latest version\n'));
}
...
function updatePackageScripts() {
  ...
  if (!fs.existsSync(packagePath)) {
    console.log(chalk.yellow('⚠️  No package.json found. Skipping script installation.'));
    return;
  }
  ...
  try {
    ...
    const qaScripts = {
      ...
      'qa:update': 'npm update @bateriku/qa-scripts',
      'qa:update:latest': 'npm install @bateriku/qa-scripts@latest',
    };
    ...
    if (scriptsAdded) {
      ...
      console.log(chalk.green('✅ QA test scripts added to package.json'));
    } else {
      console.log(chalk.gray('ℹ️  QA test scripts already present in package.json'));
    }
  } catch (error) {
